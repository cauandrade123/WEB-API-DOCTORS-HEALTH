...
const transporter = nodemailer.createTransport({
  ...
  endpoints.post('/enviar/email', (req, resp) => {
    ...
    // Verificação do campo email que é obrigatório
    if (!email) {
      return resp.status(400).send('Email não fornecido');
    }
  
    // Substitui campos opcionais por valores padrão caso estejam indefinidos
    const nomePaciente = nome || 'Paciente';
    const dataConsulta = data || 'Data não informada';
    const horarioConsulta = horario || 'Horário não informado';
